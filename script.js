...
function convert() {
  ...
  if (!input.startsWith("vpn://")) {
    if (input.startsWith("[Interface]") || input.startsWith("[Peer]")) {
      ...
      showError("Unknown format", "Expected a vpn:// key or a native AWG config.\n" + `Got: "${input.slice(0, 60)}…"`);
    }
    return;
  }
  ...
  if (typeof pako === "undefined") {
    showError(
      ...
      "pako library failed to load. Check your internet connection and reload the page.",
    );
    return;
  }
...
function hideAlerts() {
  ["alert-error", "alert-ok", "alert-warn"].forEach((id) => document.getElementById(id).classList.remove("show"));
}
